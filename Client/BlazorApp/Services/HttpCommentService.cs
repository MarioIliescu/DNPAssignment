...
public class HttpCommentService : ICommentService
{
    ...
    {
        return await _client.GetFromJsonAsync<List<CommentDto>>("comment") ?? throw new InvalidOperationException("Comments not found");
    }

    public async Task<List<CommentDto>> GetCommentFromPostAsync(int id)
    {
        return await _client.GetFromJsonAsync<List<CommentDto>>($"comment/post/{id}") ?? throw new InvalidOperationException("Comments not found");
    }
    ...
    {
        var response = await _client.PostAsJsonAsync("comment", request);
        response.EnsureSuccessStatusCode();
    ...
    {
        var response = await _client.PutAsJsonAsync("comment", request);
        response.EnsureSuccessStatusCode();
    }

    public async Task DeleteCommentAsync(int id)
    {
        var response = new HttpRequestMessage(HttpMethod.Delete, $"comment/{id}");
